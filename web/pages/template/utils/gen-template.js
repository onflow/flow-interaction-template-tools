...
export async function generateTemplate({
    ...
    argumentKeys.forEach((argKey, argKeyIndex) => {
        ...
        argKey.messages.forEach(argKeyMessage=> {
            ...
            argKeyMessage.translations.map(argKeyMessageTranslation => {
                translations[argKeyMessageTranslation.tag] = argKeyMessageTranslation.value
            })
    ...
    let depPromises = (dependencies.map(async (dep, depIndex) => {
        ...
        let depContractPromises = (dep.contracts.map(async depContract => {
            ...
            let networks = {}
            let depContractNetworksPromises = (depContract.networks.map(async depNetwork => {
                
                let latestSealedBlock = await fcl.block({ sealed: true }, { node: accessNodes[depNetwork.network] })
            ...
            for (let depContractNetworkPromise of depContractNetworksPromises) await depContractNetworkPromise
        ...
        for (let depContractPromise of depContractPromises) await depContractPromise
    }))
